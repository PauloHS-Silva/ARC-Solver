...
def glue_horizontal(grid1: Grid, grid2: Grid) -> Grid:
    ...
    assert len(grid1) == len(grid2), (
        "Grids must have the same number of rows to combine horizontally."
    )
...
def glue_vertical(grid1: Grid, grid2: Grid) -> Grid:
    ...
    assert len(grid1[0]) == len(grid2[0]), (
        "Grids must have the same number of columns to combine vertically."
    )
    return grid1 + grid2
...
def _evaluate_on_dataset_multi(
