...
from task_dsl import (
    ...
    glue_vertical,
    load_arc_task_by_id,
    evaluate_on_train_impl,
    evaluate_on_test_impl,
)
...
def solve_task(
    task_id: str, max_depth: int = 5, population_size: int = 100, time_limit: int = 60
):
    # Load the task
    ...
    problem = SingleObjectiveProblem(
        fitness_function=train_fitness_function, minimize=False
    )
    ...
    alg = GeneticProgramming(
        ...
        representation=TreeBasedRepresentation(
            grammar,
            decider=MaxDepthDecider(
                NativeRandomSource(42), grammar, max_depth=max_depth
            ),
        ),
